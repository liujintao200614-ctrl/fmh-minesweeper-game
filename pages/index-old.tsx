...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 添加调试工具到 window 对象
    (window as any).debugMetaMask = async () => {
      ...
      if ((window as any).ethereum) {
        try {
          ...
          const requestResult = await (window as any).ethereum.request({ method: 'eth_requestAccounts' });
    ...
    console.log('🛠️ Debug tool added: window.debugMetaMask()');
  }, []);
  ...
  const startNewGame = useCallback(async () => {
    ...
    if (!localGameMode && web3.isConnected && gameContract.contractsReady) {
      try {
        console.log('🚀 Attempting blockchain game initialization:', {
          isConnected: web3.isConnected,
          contractsReady: gameContract.contractsReady,
          isOnCorrectNetwork: web3.isOnCorrectNetwork,
          account: web3.account
        });
        
        const timeoutPromise = new Promise<null>((_, reject) => 
          setTimeout(() => reject(new Error('Blockchain timeout after 5 seconds')), 5000)
        );
        ...
        if (blockchainGameId && typeof blockchainGameId === 'number') {
          console.log('✅ Blockchain game started successfully with ID:', blockchainGameId);
          setGameState(prev => ({ ...prev, gameId: blockchainGameId }));
          setContractGameId(blockchainGameId);
        } else {
          console.warn('⚠️ Blockchain game start returned invalid ID:', blockchainGameId);
        }
      } catch (error) {
        console.warn('⚠️ Blockchain game start failed:', error.message);
        console.log('🎮 Continuing with local game mode');
        // Continue with local game even if blockchain fails
      }
    } else {
      console.log('🏠 Using local game mode:', {
        localGameMode,
        isConnected: web3.isConnected,
        contractsReady: gameContract.contractsReady
      });
  ...
  const handleCellClick = useCallback((row: number, col: number) => {
    ...
    if (isWon) {
      ...
      if (web3.isConnected) {
        setTimeout(async () => {
          try {
            console.log('🎯 Game completion process starting:', {
              account: web3.account,
              isConnected: web3.isConnected,
              contractGameId,
              contractsReady: gameContract.contractsReady,
            ...
            // 4. 申请链上奖励（只有满足所有条件时才执行）
            if (contractGameId && web3.isOnCorrectNetwork && gameContract.contractsReady) {
              console.log('🔗 Attempting blockchain reward claim for gameId:', contractGameId);
              ...
              console.log('⚠️ Blockchain reward claim skipped:', {
                hasGameId: !!contractGameId,
                isOnCorrectNetwork: web3.isOnCorrectNetwork,
                contractsReady: gameContract.contractsReady,
                reason: !contractGameId ? 'No game ID from blockchain' : 
                       !web3.isOnCorrectNetwork ? 'Wrong network' :
                       !gameContract.contractsReady ? 'Contracts not ready' : 'Unknown'
              });
            }
  ...
  const handleClaimReward = async () => {
    console.log('🎯 Manual reward claim attempt:', {
      contractGameId,
    ...
    if (!contractGameId || gameState.gameStatus !== 'won') {
      ...
      alert('Cannot claim reward: Game not won or no blockchain game ID');
      return;
    }

    if (!web3.isConnected || !web3.isOnCorrectNetwork || !gameContract.contractsReady) {
      console.error('Cannot claim reward: blockchain not ready', {
        isConnected: web3.isConnected,
        isOnCorrectNetwork: web3.isOnCorrectNetwork,
        contractsReady: gameContract.contractsReady
      });
      alert('Please ensure wallet is connected to the correct network and contracts are ready');
      return;
    }

    try {
      console.log('🔗 Starting manual reward claim for gameId:', contractGameId);
      await claimRewardWithSignature(contractGameId, gameState.score, gameState.timeElapsed);
      console.log('✅ Manual reward claim successful');
    } catch (error) {
      console.error('❌ Manual reward claim failed:', error);
